...
const PAGE_SIZE = 24;
const CATS = ['all','shaders','textures','mods','maps','clients','modpacks'];
...
const labels = {
  fa: {
    all:'همه', shaders:'شیدرها', textures:'تکسچر پک', mods:'مودها', maps:'مپ‌ها', clients:'کلاینت‌ها', modpacks:'مودپک‌ها',
  ...
  en: {
    all:'All', shaders:'Shaders', textures:'Texture Packs', mods:'Mods', maps:'Maps', clients:'Clients', modpacks:'Modpacks',
...
const state = {
  ...
  pages:{shaders:1,textures:1,mods:1,maps:1,clients:1,modpacks:1}
};
let items = [];
...
function applyStaticText() {
  ...
  const navMap={shaders:L.shaders,textures:L.textures,mods:L.mods,maps:L.maps,clients:L.clients,modpacks:L.modpacks};
...
function initAccounts(){
  ...
  function updateHeader(){
    ...
    btnText.textContent=a?a.username:L.login; setAvatar(btnAvatar,a?.avatar,a?.username);
  }
  function render(){
    const a=currentAccount(), L=accountLabels();
    ...
    if(a){
      form.hidden=true; profile.hidden=false; loginTab.hidden=true; registerTab.hidden=true; profileName.textContent=a.username; profileStatus.textContent=L.logged; edit.textContent=L.edit; logout.textContent=L.logout; setAvatar(profileAvatar,a.avatar,a.username); setAvatar(preview,a.avatar,a.username);
    }else{
...
// Community rating: username + mandatory 1-5 stars. Nothing can be skipped.
function initCommunityRating(){
  const form=$('#communityRatingForm'); if(!form)return;
  const user=$('#ratingUser'), result=$('#communityRatingResult');
  const key='fp:community-rating';
  function save(v){try{localStorage.setItem(key,JSON.stringify(v));}catch(e){}}
  function load(){try{return JSON.parse(localStorage.getItem(key)||'null')}catch(e){return null}}
  function labels(){return state.lang==='fa'?{title:'⭐ امتیاز بده',text:'یک نام کاربری انتخاب کن و حتماً از ۱ تا ۵ ستاره امتیاز بده.',user:'نام کاربری',choose:'یک یوزر انتخاب کن...',legend:'امتیاز شما',hint:'انتخاب ستاره اجباری است؛ امکان رد کردن وجود ندارد.',submit:'ثبت امتیاز',done:'امتیاز شما با موفقیت ثبت شد.'}:{title:'⭐ Rate First Pack',text:'Choose a username and give a mandatory rating from 1 to 5 stars.',user:'Username',choose:'Choose a username...',legend:'Your rating',hint:'A star rating is required; you cannot skip it.',submit:'Submit rating',done:'Your rating was submitted successfully.'};}
  function refresh(){
    const l=labels(); const title=$('#communityRatingTitle'), text=$('#communityRatingText'), legend=document.querySelector('.star-picker legend'), hint=$('#ratingRequiredHint'), btn=$('#communityRatingSubmit');
    if(title)title.textContent=l.title;if(text)text.textContent=l.text;if(legend)legend.innerHTML=`${l.legend} <span>*</span>`;if(hint)hint.textContent=l.hint;if(btn)btn.textContent=l.submit;
    if(user.options[0])user.options[0].textContent=l.choose;
    const old=load(); if(old&&old.user&&old.stars){user.value=old.user; const r=document.querySelector(`input[name="communityStars"][value="${old.stars}"]`);if(r)r.checked=true;result.textContent=`${old.user} • ★ ${old.stars}/5`;} else result.textContent='';
  }
  form.addEventListener('submit',e=>{
    e.preventDefault();
    const checked=form.querySelector('input[name="communityStars"]:checked');
    if(!user.value || !checked){form.reportValidity();result.textContent=state.lang==='fa'?'لطفاً حتماً یک یوزر و یک امتیاز از ۱ تا ۵ ستاره انتخاب کن.':'Please choose a username and a rating from 1 to 5 stars.';return;}
    const data={user:user.value,stars:Number(checked.value),at:new Date().toISOString()}; save(data);
    result.textContent=`${labels().done}  ${data.user} • ★ ${data.stars}/5`;
  });
  refresh();
  window.__fpCommunityRatingRefresh=refresh;
}
function initComments(){
  const form=$('#commentForm'); if(!form)return;
  const input=$('#commentInput'), list=$('#commentsList'), author=$('#commentAuthor'), avatar=$('#commentAvatar'), hint=$('#commentAccountHint'), loginHint=$('#commentLoginHint'), submit=$('#commentSubmit');
  const KEY='fp:comments';
  const load=()=>{try{return JSON.parse(safeStorage.get(KEY,'[]'))||[];}catch{return [];}};
  const save=v=>safeStorage.set(KEY,JSON.stringify(v));
  const labels=()=>state.lang==='fa'?{guest:'مهمان',login:'برای ارسال کامنت باید وارد حساب شوی.',logged:'کامنت با اکانت فعلی ثبت می‌شود.',hint:'ابتدا وارد حساب کاربری شو تا کامنت با نام و تصویر پروفایل خودت ثبت شود.',placeholder:'نظر خودت را بنویس...',submit:'ثبت کامنت',empty:'هنوز کامنتی ثبت نشده.',loginFirst:'برای کامنت گذاشتن ابتدا وارد حساب شو.',justNow:'همین الان'}:{guest:'Guest',login:'Log in to post a comment.',logged:'Your comment will use your current account.',hint:'Log in first so your comment uses your account name and profile picture.',placeholder:'Write your comment...',submit:'Post comment',empty:'No comments yet.',loginFirst:'Log in before posting a comment.',justNow:'Just now'};
  function refresh(){
    const l=labels(), a=window.__fpCurrentAccount?.();
    if(a){author.textContent=a.username;hint.textContent=l.logged;loginHint.textContent='';loginHint.hidden=true;submit.disabled=false;input.disabled=false;input.placeholder=l.placeholder;setAvatar(avatar,a.avatar,a.username);}
    else{author.textContent=l.guest;hint.textContent=l.login;loginHint.textContent=l.hint;loginHint.hidden=false;submit.disabled=true;input.disabled=true;input.placeholder=l.loginFirst;setAvatar(avatar,'',l.guest);}
    renderList(l);
  }
  function renderList(l=labels()){
    const comments=load().slice().reverse();
    if(!comments.length){list.innerHTML=`<div class="comments-empty">${esc(l.empty)}</div>`;return;}
    list.innerHTML=comments.map(c=>`<article class="comment-card"><div class="comment-head"><div class="account-avatar profile">${c.avatar?`<img src="${esc(c.avatar)}" alt="">`:esc(String(c.username||'?').slice(0,1).toUpperCase())}</div><div><strong>${esc(c.username)}</strong><time datetime="${esc(c.at)}">${esc(c.displayTime||l.justNow)}</time></div></div><p>${esc(c.text)}</p></article>`).join('');
  }
  form.addEventListener('submit',e=>{
    e.preventDefault(); const a=window.__fpCurrentAccount?.(); if(!a){showToast(labels().loginFirst);return;}
    const text=input.value.trim(); if(!text)return;
    const comments=load(); comments.push({username:a.username,avatar:a.avatar||'',text,at:new Date().toISOString(),displayTime:labels().justNow}); save(comments); input.value=''; renderList(); showToast(state.lang==='fa'?'کامنت با اکانت شما ثبت شد.':'Comment posted with your account.');
  });
  window.__fpCommentsRefresh=refresh; refresh();
}
initCommunityRating();
initAccounts();
initComments();
